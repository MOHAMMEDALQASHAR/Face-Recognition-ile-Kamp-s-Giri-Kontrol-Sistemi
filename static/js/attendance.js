...
window.addEventListener('load', () => {
    ...
    // Start polling (Timer removed for open session)
    // startCountdownTimer(); 
    startPolling();
    updateAttendanceLog();
    updateStats();
...
/**
 * Countdown Timer Functions - DISABLED FOR OPEN SESSION
 */
...
function updateTimerDisplay() {
    // Logic removed for open session
}
...
function endSessionNow() {
    Swal.fire({
        title: 'Oturumu Şimdi Sonlandır?',
        text: "Bu işlem yoklama takibini durduracak",
        icon: 'warning',
...
async function endSessionEarly() {
    const result = await Swal.fire({
        ...
        html: `
            <p>Yoklama oturumunu erken bitirmek istediğinize emin misiniz?</p>
            <p class="text-muted">Bu işlem mevcut ders için yoklamayı tamamlayacaktır.</p>
        `,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#dc3545',
        cancelButtonColor: '#6c757d',
        confirmButtonText: 'Evet, Oturumu Sonlandır',
    ...
    if (result.isConfirmed) {
        ...
        Swal.fire({
            title: 'Yoklama Tamamlanıyor...',
            html: 'Oturum verileri kaydediliyor ve istatistikler güncelleniyor.',
        ...
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Hata',
                text: 'Oturum düzgün sonlandırılamadı. Yine de yönlendiriliyor...',
                timer: 2000
            }).then(() => {
                window.location.href = '/end_session';
            });
