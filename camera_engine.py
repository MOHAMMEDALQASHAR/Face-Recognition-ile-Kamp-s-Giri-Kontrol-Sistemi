...
from db_adapter import MongoDatabaseAdapter
...
class CameraEngine:
    ...
    def start_camera(self, mode: str = "registration"):
        ...
        if self.camera is None or not self.camera.isOpened():
            # Try index 0 first (default laptop camera), fallback to 1 if fails
            # Using cv2.CAP_DSHOW for faster initialization on Windows
            self.camera = cv2.VideoCapture(0, cv2.CAP_DSHOW)
            if not self.camera.isOpened():
                print("[Camera] Camera index 0 failed, trying index 1")
    ...
    def register_student(self, name: str, student_id: str, embedding: np.ndarray) -> Tuple[bool, str]:
        """
        ...
        try:
            ...
            # 2. Check if Face already exists (using cache)
            self._update_students_cache()
            
            # Using deepface represent for similarity check logic reuse
            # or just manual cosine similarity
            if self.students_cache:
                for student in self.students_cache:
                    stored_embedding = student['embedding']
                    similarity = self._cosine_similarity(embedding, stored_embedding)
                    
                    # Threshold 0.6 is common for Facenet512 cosine similarity
                    if similarity > 0.6:
                        print(f"[Warning] Face already registered as {student['name']} ({student['student_id']})")
                        return False, 'face_exists'
